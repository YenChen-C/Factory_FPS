# Factory Strike V2.1 — 廠區戰術行動

[直接遊玩](https://yenchen-c.github.io/Factory_FPS/Factory_Strike_V2.1.html) · [原本的防線模式](https://yenchen-c.github.io/Factory_FPS/Factory_Defense_S1.14.html)
...
直接開啟網址，或下載 `Factory_Strike_V2.1.html` 後以瀏覽器開啟；所有遊戲程式、地圖、Three.js 與音效產生器均包含在單一檔案，不需安裝 Python，也不需要 CDN。瀏覽器須支援 WebGL。手機自動啟用觸控介面，建議橫向遊玩。
...
| 波次生存 | 清除全部敵人後進入下一波；陣亡結算 | 第一波 6 名、每波增加 2 名；波間補給與 20 秒準備 |
...
第一人稱使用獨立視角與照明顯示實際武器、雙手握持、後座力和換彈。九種武器含不同長度、護木、彈匣、瞄準具與刀身，HUD 同時顯示名稱及類型。手機直向會調整持槍位置。

投擲物改為按住準備、放開投擲；握持時顯示手榴彈與拉環動作，數量與引信都在投出時才開始處理。手機按住投擲鍵，可拖曳瞄準後放開；「彈種」只用來切換手榴彈／閃光／煙霧。暫停、失焦、旋轉、取消觸控或切槍會收回。

波次生存沿用舊防線的分波概念與 `4 + 波次 × 2` 敵人數。敵人分批進場，同時最多 8 名，全部清除才開始下一波；波間補滿生命、防具、彈藥與投擲物，保留位置。敵人武器與防具逐波提升；玩家陣亡即結算，可重新開始。波間 20 秒可移動準備，按 Enter 或「開始本波」提前開戰。這個新模式未移植舊防線的商店、祝福與特殊 BOSS 系統。
...
node strike/test-v21.cjs
node strike/test-viewmodel.cjs
```

`test-matches.cjs` 同時執行 36 項核心測試與四種完整 AI 對戰。其他測試涵蓋 12 項輸入情境、8 項投擲與波次情境、9 種武器與角色模型／換彈姿態，以及桌面／直向第一人稱投影與投擲姿勢。測試範圍與實際結果見 [strike/VALIDATION.md](strike/VALIDATION.md)。
