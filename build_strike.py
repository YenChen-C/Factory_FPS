...
scripts += [(P/'strike'/f).read_text() for f in ['core.js','navigation.js','audio.js','visuals.js','input.js','game.js']]
page=(P/'strike/page.html').read_text().replace('<!--SCRIPTS-->','\n'.join('<script>'+s.replace('</script','<\\/script')+'</script>' for s in scripts))
(P/'Factory_Strike_V2.0.html').write_text(page)
source_files=['src/'+f for f in ['three.min.js','config.js','models.js','factory.js','space.js']]+['strike/'+f for f in ['core.js','navigation.js','audio.js','visuals.js','input.js','game.js']]
...
(P/'index.html').write_text('<!doctype html><html lang="zh-Hant"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="0;url=Factory_Strike_V2.0.html"><title>Factory Strike</title><a href="Factory_Strike_V2.0.html">開啟 Factory Strike V2.0</a> · <a href="Factory_Defense_S1.14.html">舊版防線</a></html>')
...
print('Factory_Strike_V2.0.html:',len(page.encode()),'bytes')
