...
function makeDestruction(T,F,S,D,api){
 ...
 const bodies=[...walls,...machines,...doors],burning=new Set(),tmp=new T.Object3D();let navDirty=false;
 ...
 function maxHP(b){return b.machineName?CFG.machineHP:CFG.wallHP;}
 function resetMachines(){burning.clear();for(const b of machines){b.hp=maxHP(b);b.destroyed=false;}fire.count=core.count=smoke.count=0;}
 function reset(){for(const b of doors){b.hp=CFG.wallHP;b.disabled=b.destroyed=false;b.door.destroyed=false;b.door.group.visible=true;b.door.t=b.door.target=0;S.doorPoly(b.door);}for(const b of walls){b.hp=maxHP(b);b.destroyed=false;b.disabled=false;F.wallMeshes[b.wallIndex].visible=true;}resetMachines();navDirty=true;}
 function hit(h,damage){const b=h?.body;if(!b||!bodies.includes(b)||b.disabled||b.destroyed||!Number.isFinite(damage)||damage<=0)return false;
  b.hp=Math.max(0,b.hp-damage);if(b.hp===0){b.destroyed=true;if(b.machineName){burning.add(b);api.notify(b.machineName+' 已摧毀 · 燃燒中',1.5);}else{b.disabled=true;if(b.door){b.door.destroyed=true;b.door.group.visible=false;}else F.wallMeshes[b.wallIndex].visible=false;api.onWallDestroyed?.();navDirty=true;api.notify(b.door?'門已擊破':'牆面已擊破',1);}}
  return true;
 }
 ...
 function tick(time){if(navDirty){S.rebuild();S.flow(api.player.pos);navDirty=false;}
 ...
 function label(h){const b=h?.body;if(!b||!bodies.includes(b)||b.disabled)return '';return (b.machineName||(b.door?'門':'牆面'))+' · '+(b.destroyed?'已摧毀／燃燒中':Math.ceil(b.hp)+' / '+maxHP(b)+' HP');}
 ...
 reset();return {nearBurn,hit,blast,tick,reset,resetMachines,label,walls,machines,doors,burning};
}
