## Factory Strike V2.0 — 2026-09-23

新增獨立的廠區戰術 FPS 入口：四種 AI 隊伍模式、七種主武器、手槍與刀、傷口與防具、蹬牆與滑鏟、三種投擲物、配裝與戰績、原創程序化戰術視覺、空間音效與自動手機操作。保留舊版 S1.14 防線入口。

保留完整廠區幾何；導航使用實際牆、設備與開啟門片的碰撞檢查，避免路線切過窄角。CTF 分開旗點與重生區，攜旗後立即換路返回；爆破雙目標分開至不同通道，AI 能完成安裝與引爆。

單一離線 HTML 約 1.06 MB。首頁改為戰術模式。程式測試通過；實機 GPU 與手機觸控手感尚未驗證，詳見 `strike/VALIDATION.md`。
