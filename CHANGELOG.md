## Factory Strike V2.1 — 2026-09-23

- 重建第一人稱武器視角、照明、手套與手臂，調整九種武器的辨識細節、手機直向位置及刀的握持角度。
- 手榴彈／閃光／煙霧改為按住準備、放開投擲，包含拉環、持握與出手動畫；取消操作不扣庫存，握持不倒數引信。
- 新增波次生存：首波 6 人，每波 +2，分批出現且最多 8 人同場；清波後補給、20 秒準備，可提前開始，死亡結算。
- 擴充輸入、波次、投擲狀態及第一人稱投影測試；保留 V2.0 與舊防線。
- 新入口 `Factory_Strike_V2.1.html`，仍為可離線開啟的單一 HTML。
