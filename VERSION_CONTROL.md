...
目前最新入口為 **Factory Strike V2.1**。舊版防線 S1.14 保留於原網址。
...
3. 新戰術版執行 `node strike/test-matches.cjs`、`node strike/test-input.cjs`、`node strike/test-visuals.cjs`、`node strike/test-v21.cjs`、`node strike/test-viewmodel.cjs`。修改共用 `src/` 時，亦需執行根目錄相關 `test*.cjs`。
...
正式遊玩：https://yenchen-c.github.io/Factory_FPS/Factory_Strike_V2.1.html
